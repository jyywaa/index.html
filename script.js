// DATA PRODUK (GAMBAR BAJU & CELANA SUDAH DISESUAIKAN PRESISI & HD)
const products = [
...
let listPesanan = JSON.parse(localStorage.getItem('mochi_pesanan')) || [
  { id: 101, pembeli: "Syera", hp: "081299887766", alamat: "Jl. Dahlia No 12, Bandung", items: [{nama:"Nasi Goreng", qty:1}], total: 20000, status: "dikemas", tgl: "2026-08-19" }
];
...
function tambahBarang() {
  ...
  listBarang.push({
    ...
    img: kat==='outfit'?"https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=500":"https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=500", desc: "Produk berkualitas MochiShop"
  });
  ...
  container.innerHTML = list.map(p => `
    <div class="card-cus" onclick="showDetailProduct(${p.id})">
      ...
      <div class="info">
        ...
        <button class="card-btn" onclick="event.stopPropagation(); addCart(${p.id})">
          <i class="fa-solid fa-cart-shopping" style="color:white;"></i> Beli
        </button>
      </div>
    </div>
  `).join('');
...
function showDetailProduct(id) {
  ...
  document.getElementById('detailContent').innerHTML = `
    ...
    <button class="btn" style="margin-top:10px;" onclick="addCart(${p.id})">
      <i class="fa-solid fa-cart-shopping" style="color:white;"></i> Tambah Ke Keranjang
    </button>
    ...
    ${sudahBeli ? `
      <div style="background:#FFF0F7; padding:12px; border-radius:12px; margin-top:8px;">
        ...
        <textarea id="inputKomentarText" rows="2" style="margin-top:6px;"></textarea>
